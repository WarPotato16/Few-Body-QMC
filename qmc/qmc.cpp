...
SignedLogWeight signedLogConfigurationWeight(
    ...
    for (const BondOperator& op : operators) {
        ...
        activeCount += 1;
        LocalMatrixElement elem = applyModelOperator(state, op, bonds, model);
...
bool diagonalUpdateAttempt(
    ...
    std::uniform_int_distribution<int> bondDist(0, bondCount - 1);
    ...
    if (oldOperator.kind == OperatorKind::Identity) {
        ...
        newOperator.bond = bondDist(rng);
        proposalLogRatio = std::log(static_cast<double>(bondCount));
...
QmcResult runWormQmc(const QmcParameters& params) {
    ...
    for (const MeasurementBlock& block : blocks) {
        ...
        blockEnergies.push_back(-block.signedActiveSum / (params.beta * block.signSum));
    }
    ...
    if (std::abs(signTotal) < 1e-14) {
        ...
        result.energy = -signedActiveTotal / (params.beta * signTotal);
...
int main(int argc, char** argv) {
    ...
    for (int particleCount : particleNumbers) {
        ...
        double exactValue = referenceEnergyValue(
            params.model,
            params.boundary,
            params.latticeSize,
            params.beta,
